...
from crud.forms import stforms
...
def stinsert(request):
    if request.method=="POST":
        if request.POST.get('stname') and request.POST.get('stemail') and request.POST.get('staddress') and request.POST.get('stmobile') and request.POST.get('stgender'):
            ...
            savest.save()
            messages.success(request,"The Record" +savest.stname+"saved successfully..!")
...
def stupdate(request,id):
    ...
    form=stforms(request.POST,instance=stupdate)
    if form.is_valid():
        form.save()
        messages.success(request,"The student Record is updated Successfuly")
